...
\begin{tikzpicture}[node distance=2.5cm]
    ...
    % Transitions
    % From Invalid
    \draw [arrow] (I) -- node[midway, left] {Local Read \& No Other Copy} (E);
    \draw [arrow] (I) -- node[midway, above, sloped] {Local Read \& Other Copy} (S);
    \draw [arrow] (I) -- node[midway, below, sloped] {Local Write} (M);
    
    % From Shared
    \draw [arrow] (S) -- node[midway, left] {Local Write (BusRdX)} (M);
    \draw [arrow] (S) -- node[midway, right] {Remote BusRdX} (I);
    
    % From Exclusive
    \draw [arrow] (E) -- node[midway, above] {Local Write} (M);
    \draw [arrow] (E) -- node[midway, right] {Remote BusRd} (S);
    \draw [arrow] (E) -- node[midway, above, sloped] {Remote BusRdX} (I);
    
    % From Modified
    \draw [arrow] (M) -- node[midway, left, align=center] {Remote BusRd\\(Write back)} (S);
    \draw [arrow] (M) -- node[midway, above, sloped, align=center] {Remote BusRdX\\(Write back)} (I);
    \draw [arrow] (M) to [out=135,in=90,looseness=0.8] node[midway, above, align=center] {Eviction\\(Write back)} (I);
\end{tikzpicture}
\end{adjustbox}
